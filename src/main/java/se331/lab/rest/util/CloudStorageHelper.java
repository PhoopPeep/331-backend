...
public class CloudStorageHelper {
    ...
    static {
        ...
        try{
            serviceAccount = new ClassPathResource("a309889cab1a601ebd6f76c85fed09c73684105d.json").getInputStream();
            storage = StorageOptions.newBuilder().setCredentials(GoogleCredentials.fromStream(serviceAccount)).setProjectId("imageupload-2b511").build().getService();
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
...
//      // Convert ByteArrayOutputStream into byte[]
        BlobInfo blobInfo =
                storage.create(BlobInfo.newBuilder(bucketName, fileName).setAcl(new ArrayList<>(Arrays.asList(Acl.of(Acl.User.ofAllUsers(), Acl.Role.READER)))).setContentType(filePart.getContentType()).build(),os.toByteArray());
